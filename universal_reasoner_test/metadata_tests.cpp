...
TEST(BasicMetadata, AddingFacts)
{
	...
	auto  ress2 = repo->GetFactByName<std::string>("test3");
...
TEST(BasicMetadata, AddingRules)
{
	...
	repo->AddFact(FactSettable<std::string>(), "test3");
 	...
 	auto s3 = repo->GetSettableFactByName<std::string>("test3");
	repo->AddFact(2.0, "d1");
	repo->AddFact(2, "i1");


	auto conclusion1 = std::make_shared<ConclusionSettingFact<std::string>>(s3, "test3");


 	auto premis1 = std::make_shared<PremiseWithType<std::string>>(repo->GetFactByName<std::string>("test1"), "test1");

	auto  ress3 = repo->GetFactByName<std::string>("test3");
	EXPECT_THROW(ress3->GetValue(), std::logic_error);

	auto rule1 = std::make_shared<RuleImpl<double>>(premis1, conclusion1);
	data.AddRule(rule1);
